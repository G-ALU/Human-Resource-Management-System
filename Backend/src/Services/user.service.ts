...
export class userService {
  ...
  async fetchAllUsers(){
    ...
    let pool = await mssql.connect(sqlconfig)
    let result = (await pool.query(`SELECT * FROM Users WHERE role = 'user' AND status = 'active'`)).recordset
  ...
  async fetchManagers(){
    let pool = await mssql.connect(sqlconfig)
    let result = (await pool.query(`SELECT * FROM Users WHERE role ='manager' AND status = 'active'`)).recordset
  ...
  async fetchsinglemanager(user_id:string){
    ...
    let user = (
      await pool
        .request()
        .input("user_id", mssql.VarChar, user_id)
        .query(`SELECT * FROM Users WHERE role = 'manager' AND user_id = '${user_id}'`)
    ).recordset;
  ...
  async switchRoles(user_id: string) {
    ...
    if (response.user.user_id) {
      ...
      if (response[0] < 1) {
        ...
        return {
          message: "User role changed successfully to manager",
        };
      }
    } else {
      return {
  ...
  async switchRoles2(user_id: string) {
    let response = await this.fetchSingleUser(user_id);
    ...
    if (response.user.user_id) {
      ...
      let response = await (
        await pool
          ...
          .input("user_id", mssql.VarChar, user_id)
          .query(
            `UPDATE Users SET role = 'user' WHERE role = 'manager' AND user_id = '${user_id}'`
          )
      ).rowsAffected;
      ...
      if (response[0] < 1) {
        ...
        return {
          message: "User role changed successfully to employee ",
        };
      }
    } else {
      return {
...
async reactiveUser(user_id: string){
  try {
    let pool =  await mssql.connect(sqlconfig)
    let userExists = (await pool.request()
        .input('user_id', mssql.VarChar, user_id)
